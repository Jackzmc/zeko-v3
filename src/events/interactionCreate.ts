...
import { Interaction, Client, AutocompleteInteraction, InteractionType } from 'discord.js'
...
export default class extends CoreEvent {
    ...
    async every(interaction: Interaction) {
        if(interaction.isButton()) {
        ...
        } else if(interaction.type == InteractionType.ApplicationCommandAutocomplete || interaction.type == InteractionType.ApplicationCommand) {
            ...
            if(interaction.type == InteractionType.ApplicationCommandAutocomplete) { 
                ...
                try {
                    if(handler) {
                        await handler.bind(slash.command)(autocomplete, focused)
                    } else {
                ...
                try {
                    ...
                    try {
                        if(handler)
                            await handler.bind(slash.command)(interaction, options)
                        else
